...
function clone(source,to,cb){
	var git = exec(format('git clone',source,wrapInQuotes(to)), function(err, stdout){
        if(err) return cb(err);
        var git = path.join(to, '.git');
        try {
            rmdir(wrapInQuotes(git));
        } catch (e) {
            console.log('Failed to remove .git directory from newly cloned project. You will want to manually remove this directory, friend!');
        }
